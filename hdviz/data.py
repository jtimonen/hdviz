...
def create_pointdata(x: np.ndarray, categories=None):
    assert_dim(x, 2)
    return PointData(x, categories)


def create_linedata(x: np.ndarray, categories=None):
    assert_dim(x, 3)
    return LineData(x, categories)
...
class PlotData:
    ...
    def __init__(self, categories, num_objects):
        if categories is None:
            categories = "group 1" * num_objects
